...
const build = {
  ...
  scrub: {
    ...
    client: rimraf('build/client'),
    server: rimraf('build/server'),
  },
  development: {
    default: series.nps('commit.pre','build.development.multi'),
    client: series('nps build.scrub.client', 'npx webpack --config '+_paths.wp.client.dev),
    server: series('nps build.scrub.server', 'npx webpack --config '+_paths.wp.server.dev),
    multi: concurrentBuild('development'),
  },
  production: {
    default: series.nps('commit.pre', 'scrub.build', 'build.production.multi'),
    client: series('nps build.scrub.client', 'npx webpack --config '+_paths.wp.client.prod),
    server: series('nps build.scrub.server', 'npx webpack --config '+_paths.wp.server.prod),
